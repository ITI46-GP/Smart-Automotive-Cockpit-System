// Ported from 02-Digital-Cluster/Gauge/GaugeActiveArc.qml — already
// Shape + CurveRenderer, no Canvas, no layer.enabled. Per PLAN.md's audit
// this component was already compliant with the perf rules before this
// rebuild even started, so it's ported close to verbatim.
//
// S2: instantiated with a STATIC value (see Main.qml) — no animation yet.
// S3 will animate `value` over time; this component's smoothing Behavior
// already exists and will kick in then without any change here.
//
// Theme is a real C++ QML_SINGLETON now (src/Theme.h), registered in the
// QnxCluster module — same proven pattern the old app used for its Backend
// singletons. That means it DOES need an explicit module import here
// (C++-registered module types, unlike same-module composite .qml types,
// aren't visible without importing the module by URI).
